...
void print_samples(const WDL_HeapBuf* const firmware, int ofs, const int num)
{
	...
	static const char* const hdr  = "Offset  | Atn | PC   | PF   | -  | Attack | -  | Loop   | Addr   | F | L8 | ?  | N1  | N2  ";
	static const char* const line = "--------+-----+------+------+----+--------+----+--------+--------+---+----+----+-----+-----";
...
int main(const int argc, const char* const* const argv)
{
	...
	{
		// IC2 XR560B0 ROM 4M
		"db60xg/xr560b0.ic7", // SHA1(d17e4ac0fc0e50a06d9d505bc87af155356436eb)

		// IC6 XQ730B0 WAVE ROM 1 16M
		"db60xg/xq730b0.ic9", // SHA1(32f653c7644d060f5a6d63a435ae3a7412386d92)
		// IC7 XQ731B0 WAVE ROM 2 16M
		"db60xg/xq731b0.ic10" // SHA1(adf68689b4842ec5bc9b0ea1bb99cf66d2dec4de)
	};
	...
	if (opt == '-f')
	{
		const int size = write_firmware("build/db60xg_firmware.bin", &firmware);
		return size == firmware.GetSize() ? EXIT_SUCCESS : EXIT_FAILURE;
	}

	if (opt == '-t')
	{
		printf("DB60XG Data Tables\n\n");

		print_drum_banks(&firmware, +270652, 3); puts("\n--\n");
		print_drum_kits(&firmware, +281628, 23); puts("\n--\n");
		print_drum_voices(&firmware, +271038, 353); puts("\n--\n");
		print_sfx_voices(&firmware, +147366, +317238, 87); puts("\n--\n");

		print_bank_lists(&firmware, +267692, 3); puts("\n--\n");
		print_program_banks(&firmware, +249516, 71); puts("\n--\n");
		print_normal_voices(&firmware, +147366, 743); puts("\n--\n");

		print_sample_sets(&firmware, +305772, 247); puts("\n--\n");
		print_samples(&firmware, +287516, 1141);
	...
	if (opt == '-w')
	{
		int n = extract_drum_samples("wave/db60xg/drum_%05d.wav", &firmware, +271038, 353, &wavetbl);
		n += extract_samples("wave/db60xg/sample_%05d.wav", &firmware, +287516, 1141, &wavetbl);
