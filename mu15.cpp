...
void print_bank_lists(const WDL_HeapBuf* const firmware, const int ofs, const int num, const char* const title = NULL)
{
	...
	for (int i = 0, j = c == 'X' ? 0 : INT_MAX; i < num; ++i)
	{
		putchar('\n');
...
int read_wavetbl(const char* const filename, WDL_HeapBuf* const buf, int size, const int ofs = 0)
{
	size -= ofs;
	...
	int n = ofs ? !fseek(fp, ofs, SEEK_SET) : 1;
	if (n) n = (int)fread(ptr, 1, size, fp);
	fclose(fp);
...
int extract_sample(const char* const filename, const int ofs, const WDL_HeapBuf* const wavetbl, const unsigned char* const ptr)
{
	...
	static const int max_samples = 614;
...
int main(const int argc, const char* const* const argv)
{
	...
	{
		// IC10 XV684C0 ROM 32M 98-11-28
		"mu15/xv684c0.bin" // SHA1(e286f83ed1fb90e0f98fe565b58112da18f88b5a)
	};

	WDL_HeapBuf firmware;

	if (!read_firmware(roms[0], &firmware, +448252))
	{
		return EXIT_FAILURE;
	}
	...
	if (opt == '-f')
	{
		const int size = write_firmware("build/mu15_firmware.bin", &firmware);
		return size == firmware.GetSize() ? EXIT_SUCCESS : EXIT_FAILURE;
	}

	if (opt == '-m')
	{
		const int size = write_midi("midi/mu15_demo.mid", &firmware, +191752);
		const int n = size > 0;
	...
	if (opt == '-t')
	{
		printf("MU15 Data Tables\n\n");

		print_drum_banks(&firmware, +376588, 3); puts("\n--\n");
		print_drum_kits(&firmware, +370868, 22); puts("\n--\n");
		print_drum_voices(&firmware, +361448, 314); puts("\n--\n");
		print_sfx_voices(&firmware, +360972, 87); puts("\n--\n");

		print_bank_lists(&firmware, +360716, 2, "XG"); puts("\n--\n");
		print_bank_lists(&firmware, +361320, 1, "TG300B"); puts("\n--\n");
		print_program_banks(&firmware, +342796, 70); puts("\n--\n");
		print_normal_voices(&firmware, +247086, 683); puts("\n--\n");

		print_sample_sets(&firmware, +246594, 246); puts("\n--\n");
		print_samples(&firmware, +228338, 1141);
	...
	if (opt == '-b')
	{
		FILE* const fp = fopen("table/mu15_bitmap.txt", "wb");
		if (!fp) return EXIT_FAILURE;

		static const unsigned short bom = 0xFEFF;
		fwrite(&bom, 2, 1, fp);

		write_utf16_str("MU15 Bitmaps\n\n", fp);

		int n = print_bitmaps(fp, &firmware, +439380, 86);
		n += print_bitmaps(fp, &firmware, +442478, 4);
	...
	if (!read_wavetbl(roms[0], &wavetbl, 4*1024*1024, firmware.GetSize()))
	{
		return EXIT_FAILURE;
	}
	...
	if (opt == '-w')
	{
		int n = extract_drum_samples("wave/mu15/drum_%05d.wav", &firmware, +361448, 314, &wavetbl);
		n += extract_samples("wave/mu15/sample_%05d.wav", &firmware, +228338, 1141, &wavetbl);
