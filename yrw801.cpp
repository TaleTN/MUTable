// Copyright (C) 2020-2024 Theo Niessink <theo@taletn.com>
...
--------+------+-----------------+------------------------------+--------------------------+---------
+0      | 1    | Bit 6  - 7      | Sample format                | 0:8, 1:12, 2:16-bit      |
...
int write_sample(const char* const filename, const WDL_HeapBuf* const wavetbl, const int format, const int addr, int loop, const int end)
{
	...
	WaveWriter wav;
	if (!wav.Open(filename, /* format == 0 ? 8 : */ 16, 1, 44100, 0)) return 0;
...
int main(const int argc, const char* const* const argv)
{
	...
	{
		"yrw801/yrw801.rom" // SHA1(32760893ce06dbe3930627755ba065cc3d8ec6ca)
	};

	WDL_HeapBuf wavetbl;

	if (!read_wavetbl(roms[0], &wavetbl, 2*1024*1024))
	{
		return EXIT_FAILURE;
	}
	...
	if (opt == '-t')
	{
		printf("YRW801 Data Tables\n\n");
		print_wavetbl(&wavetbl, +0, 384);
	...
	if (opt == '-w')
	{
		const int n = extract_samples("wave/yrw801/sample_%04d.wav", &wavetbl, +0, 330);
