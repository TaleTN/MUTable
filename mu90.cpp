...
int extract_sample(const char* const filename, const int ofs, const WDL_HeapBuf* const wavetbl, const unsigned char* const ptr)
{
	...
	static const int max_samples = 1031;
...
int main(const int argc, const char* const* const argv)
{
	...
	{
		// IC9 XS519D0 8M v1.01
		"mu90/xs519d0.ic9", // SHA1(05068eddcaf5be3a57f3a412a95c204849011b34)

		// IC22 XS518A0 WAVE ROM 1 32M
		"mu90/xs518a0.ic22", // SHA1(fd3cce228c7d389a2fde25c808a5b26080588cba)
		// IC23 XS743A0 WAVE ROM 2 32M
		"mu90/xs743a0.ic23"  // SHA1(a67bb49378a38a2d809bd717d286e18bc6496db0)
	};
	...
	if (opt == '-f')
	{
		const int size = write_firmware("build/mu90_firmware.bin", &firmware);
		return size == firmware.GetSize() ? EXIT_SUCCESS : EXIT_FAILURE;
	}

	if (opt == '-m')
	{
		const int size = write_midi("midi/mu90_demo.mid", &firmware, +761043);
		const int n = size > 0;
	...
	if (opt == '-t')
	{
		printf("MU90 Data Tables\n\n");

		print_drum_banks(&firmware, +618718, 3); puts("\n--\n");
		print_drum_kits(&firmware, +610782, 31); puts("\n--\n");
		print_drum_voices(&firmware, +589824, 499); puts("\n--\n");
		print_sfx_voices(&firmware, +637918, 88); puts("\n--\n");

		print_bank_lists(&firmware, +637534, 3); puts("\n--\n");
		print_program_banks(&firmware, +619102, 72); puts("\n--\n");
		print_normal_voices(&firmware, +638094, 786); puts("\n--\n");

		print_sample_sets(&firmware, +760444, 294); puts("\n--\n");
		print_samples(&firmware, +734924, 1595);
	...
	if (opt == '-b')
	{
		FILE* const fp = fopen("table/mu90_bitmap.txt", "wb");
		if (!fp) return EXIT_FAILURE;

		static const unsigned short bom = 0xFEFF;
		fwrite(&bom, 2, 1, fp);

		write_utf16_str("MU90 Bitmaps\n\n", fp);
		const int n = print_bitmaps(fp, &firmware, +270586, 708);
	...
	if (!read_wavetbl(&roms[1], &wavetbl, 2, 4*1024*1024))
	{
		return EXIT_FAILURE;
	}
	...
	if (opt == '-w')
	{
		int n = extract_drum_samples("wave/mu90/drum_%05d.wav", &firmware, +589824, 499, &wavetbl);
		n += extract_samples("wave/mu90/sample_%05d.wav", &firmware, +734924, 1595, &wavetbl);
