...
void print_drum_voices(const WDL_HeapBuf* const firmware, int ofs, const int num)
{
	...
	static const char* const hdr  = "                                                                  Voice                                                                  |                                       Sample                                        \n"
	                                "Offset  | ?  | PF  | Lvl | Alt | Pan | Rev | Cho | Var | K | 0 | 1 | Cut | Q   | A   | D1  | D2  | LG  | HG  | LF | HF  | ?        | PC  | SFX#  | Rat | PF   | PC   | ?     | Attack   | R | ?  | Loop     | F | D8 | Address ";
...
int main(const int argc, const char* const* const argv)
{
	...
	{
		// Firmware v2.01
		"mu1000/mu1000-v2.01-h.bin", // SHA1(cee47062966b01ce72e8ebaf6f7fa9778b32f6ab)
		"mu1000/mu1000-v2.01-l.bin", // SHA1(19f51c6304e3550d0bb8b3cca647f1fc609b0994)
	...
	if (opt == '-f')
	{
		const int size = write_firmware("build/mu1000_firmware.bin", &firmware);
		return size == firmware.GetSize() ? EXIT_SUCCESS : EXIT_FAILURE;
	}

	if (opt == '-t')
	{
		printf("MU1000 Data Tables\n\n");

		print_drum_banks(&firmware, +1925816, 6); puts("\n--\n");
		print_drum_kits(&firmware, +1910214, 60); puts("\n--\n");
		print_drum_voices(&firmware, +1867080, 1027); puts("\n--\n");
		print_sfx_voices(&firmware, +1866440, 97); puts("\n--\n");

		print_bank_lists(&firmware, +1865928, 4, "MU1000"); puts("\n--\n");
		print_bank_lists(&firmware, +1866952, 1, "TG300B"); puts("\n--\n");
		print_bank_lists(&firmware, +1926584, 2, "GM"); puts("\n--\n");
		print_program_banks(&firmware, +1752776, 221); puts("\n--\n");
		print_normal_voices(&firmware, +1507032, 1635); puts("\n--\n");

		print_sample_sets(&firmware, +1506024, 503); puts("\n--\n");
		print_samples(&firmware, +1459608, 2901);
	...
	if (opt == '-b')
	{
		...
		FILE* const fp = fopen("table/mu1000_bitmap.txt", "wb");
		if (!fp) return EXIT_FAILURE;

		static const unsigned short bom = 0xFEFF;
		fwrite(&bom, 2, 1, fp);

		write_utf16_str("MU1000 Bitmaps\n\n", fp);
	...
	if (opt == '-w')
	{
		int n = extract_drum_samples("wave/mu1000/drum_%05d.wav", &firmware, +1867080, 1027, &wavetbl);
		n += extract_samples("wave/mu1000/sample_%05d.wav", &firmware, +1459608, 2901, &wavetbl);
