...
int main(const int argc, const char* const* const argv)
{
	...
	{
		// IC7 XS289A0 4M v0.05
		"mu10/xs289a0.ic07", // SHA1(4c2bb0c53a756d64cf6315b85642cab3b86e39fc)

		// IC11 XR709A0 32M
		"mu10/xr709a0.ic11"  // SHA1(5286b933976f9e3a9ca82523b8d591b616cf9479)
	};
	...
	if (opt == '-f')
	{
		const int size = write_firmware("build/mu10_firmware.bin", &firmware);
		return size == firmware.GetSize() ? EXIT_SUCCESS : EXIT_FAILURE;
	}

	if (opt == '-t')
	{
		printf("MU10 Data Tables\n\n");

		print_drum_banks(&firmware, +272528, 3); puts("\n--\n");
		print_drum_kits(&firmware, +283504, 23); puts("\n--\n");
		print_drum_voices(&firmware, +272914, 353); puts("\n--\n");
		print_sfx_voices(&firmware, +149242, +320292, 87); puts("\n--\n");

		print_bank_lists(&firmware, +269568, 3); puts("\n--\n");
		print_program_banks(&firmware, +251392, 71); puts("\n--\n");
		print_normal_voices(&firmware, +149242, 743); puts("\n--\n");

		print_sample_sets(&firmware, +307648, 247); puts("\n--\n");
		print_samples(&firmware, +289392, 1141);
	...
	if (!read_wavetbl(roms[1], &wavetbl, 4*1024*1024))
	{
		return EXIT_FAILURE;
	}
	...
	if (opt == '-w')
	{
		int n = extract_drum_samples("wave/mu10/drum_%05d.wav", &firmware, +272914, 353, &wavetbl);
		n += extract_samples("wave/mu10/sample_%05d.wav", &firmware, +289392, 1141, &wavetbl);
