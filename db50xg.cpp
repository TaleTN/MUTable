...
int main(const int argc, const char* const* const argv)
{
	...
	{
		// IC2 XR253A0 ROM 4M
		"db50xg/xr253a0.ic7", // SHA1(61a80e066c7ff40dcba01b796d9faff3b9952a5b)

		// IC9 XQ730B0 WAVE ROM 1 16M
		"db50xg/xq730b0.ic9", // SHA1(32f653c7644d060f5a6d63a435ae3a7412386d92)
		// IC10 XQ731B0 WAVE ROM 2 16M
		"db50xg/xq731b0.ic10" // SHA1(adf68689b4842ec5bc9b0ea1bb99cf66d2dec4de)
	};
	...
	if (opt == '-f')
	{
		const int size = write_firmware("build/db50xg_firmware.bin", &firmware);
		return size == firmware.GetSize() ? EXIT_SUCCESS : EXIT_FAILURE;
	}

	if (opt == '-t')
	{
		printf("DB50XG Data Tables\n\n");

		print_drum_banks(&firmware, +262676, 3); puts("\n--\n");
		print_drum_kits(&firmware, +273652, 23); puts("\n--\n");
		print_drum_voices(&firmware, +263062, 353); puts("\n--\n");
		print_sfx_voices(&firmware, +139390, +309112, 87); puts("\n--\n");

		print_bank_lists(&firmware, +259716, 3); puts("\n--\n");
		print_program_banks(&firmware, +241540, 71); puts("\n--\n");
		print_normal_voices(&firmware, +139390, 743); puts("\n--\n");

		print_sample_sets(&firmware, +297796, 247); puts("\n--\n");
		print_samples(&firmware, +279540, 1141);
	...
	if (opt == '-w')
	{
		int n = extract_drum_samples("wave/db50xg/drum_%05d.wav", &firmware, +263062, 353, &wavetbl);
		n += extract_samples("wave/db50xg/sample_%05d.wav", &firmware, +279540, 1141, &wavetbl);
